...
from discord.ext.commands import command, has_role, Context
...
class CogCanales(CategoriaComandos):
    ...
    @command(name='listach', aliases=['lsch'], help='Mostrar lista de canales que el bot escucha')
    ...
    @command(name='agregarch', aliases=['addch'], help='Agregar canal al que el bot escucha (Mención al canal para uno en específico)')
    @has_role(DEV_ROLE_ID)
    ...
    @command(name='eliminarch', aliases=['removech', 'delch'], help='Remover un canal al que el bot escucha (Mención al canal para uno en específico)')
    @has_role(DEV_ROLE_ID)
